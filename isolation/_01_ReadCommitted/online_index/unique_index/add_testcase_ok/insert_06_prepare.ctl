/* 
Test Case: insert & prepare
prepare statement to insert duplicate data and commit while loading index with online;
...
C3: prepare st from 'insert into t1(b,c) values(?,?)';
C3: COMMIT;
MC: wait until C3 ready;

C4: prepare st1 from 'update t1 set b=?,c=? where b=6';
C4: COMMIT;
...
C3: execute st using 1,'a';
MC: wait until C3 blocked;

C4: execute st1 using 8,'gg';
